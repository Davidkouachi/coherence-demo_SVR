...
class RisqueController extends Controller
{
    ...
    public function add_risque(Request $request)
    {
        ...
        $risque->date_validation = now()->format('Y-m-d\TH:i');
        $risque->statut = 'valider';
        ...
        if ($rechs) {
            ...
            foreach ($rechs as $value) {
                ...
                $rech = Suivi_action::where('action_id', $value->action_id)->count();

                if($rech === 0) {

                    $suivip = new Suivi_action();
                    $suivip->statut = 'non-realiser';
                    $suivip->action_id = $value->action_id;
                    $suivip->save();
                }
            }
        }
